...
public class RoomManager {
	List<gameServer.RoomThread> room_threads = new ArrayList<RoomThread>();
	...
	public void moveToRoom(int room_id, acceptanceServer.Client client) {
		// guestならidは-1なので，空いているところに放り込む
		if(room_id == -1) {
			try{
				room_id = getVacantRoom();
			} catch(acceptanceServer.NoVacantRoomException e) {
		...
		gameServer.Player player = new Player(client);
	 ...
	 */
	private int getVacantRoom() throws acceptanceServer.NoVacantRoomException {
